...
const Card = ({ imageUrl, eventName, trainer, time, days, date, location, theme }) => {
  ...
  const handleLike = () => {
    setLiked(!liked);
    setLikeCount(likeCount + (liked ? -1 : 1)); // Increment or decrement the like count
    if (disliked) {
      setDisliked(false);
      setDislikeCount(dislikeCount - 1); // Adjust the dislike count if it was previously active
    }
  };

  const handleDislike = () => {
    setDisliked(!disliked);
    setDislikeCount(dislikeCount + (disliked ? -1 : 1)); // Increment or decrement the dislike count
    if (liked) {
      setLiked(false);
      setLikeCount(likeCount - 1); // Adjust the like count if it was previously active
    }
  };

  const handleBookmark = () => {
    setBookmarked(!bookmarked);
    setBookmarkCount(bookmarkCount + (bookmarked ? -1 : 1)); // Increment or decrement the bookmark count
  };
  ...
  return (
    <div className={`workoutCard ${theme}`}>
      <button className="workoutImageButton">
        ...
        <h2 className="trainer">Trainer: {trainer}</h2>
        <h2 className="days">Days: {days}</h2>
          ...
          <div className="details">
            <div className="iconContainer">
              <div className="iconWrapper">
                ...
                <h2 className="likeCounter">{likeCount}</h2>
              </div>
              <div className="iconWrapper">
                ...
                <h2 className={`bookmarkCounter ${bookmarked ? 'bookmerked' : ''}`} onClick={handleBookmark}>{bookmarkCount}</h2>
              </div>
              <div className="iconWrapper">
                ...
                <h2 className="dislikeCounter">{dislikeCount}</h2>
              </div>
            </div>
          </div>
        </div>
